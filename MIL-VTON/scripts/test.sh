...
if [ $1 == 1 ]; then
   python3 test_warping.py \
    ...
    --PBAFN_warp_checkpoint '/home/ubuntu/GP-VTON/checkpoints/checkpoints/gp-vton_partflow_vitonhd_usepreservemask_lrarms_1027/PBAFN_warp_epoch_121.pth' \
    --resize_or_crop None --verbose --tf_log \
    --dataset vitonhd --resolution 512 \
    --batchSize 2 --num_gpus 1 --label_nc 14 \
    --dataroot /home/ubuntu/GP-VTON/dataset/VITON-HD-512 \
    --image_pairs_txt test_pairs_paired_1018.txt

### for try-on generation
elif [ $1 == 2 ]; then
   python3 -m torch.distributed.launch --nproc_per_node=1 --master_port=4736 test_tryon.py \
        --name test_gpvtongen_vitonhd_paired_1109 \
        --resize_or_crop None --verbose --tf_log \
        --dataset vitonhd --resolution 512 \
        --batchSize 2 --num_gpus 1 --label_nc 14 --launcher pytorch \
        --PBAFN_gen_checkpoint '/home/ubuntu/GP-VTON/checkpoints/checkpoints/VITON-HD/gp-vton_gen_vitonhd_wskin_wgan_lrarms_1029/PBAFN_gen_epoch_201.pth' \
        --dataroot /home/ubuntu/GP-VTON/dataset/VITON-HD-512 \
        --image_pairs_txt test_pairs_paired_1018.txt\
...
### for garment warping
   ...
   python test_warping.py \
    ...
    --PBAFN_warp_checkpoint '../PBAFN_warp_epoch_051.pth' \
    --resize_or_crop None \
    --verbose --tf_log \
    --dataset dresscode \
    --resolution 512 \
